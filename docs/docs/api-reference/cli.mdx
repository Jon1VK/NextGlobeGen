...
The `next-globe-gen generate` command runs the generator which generates the localized routes, compiles messages and generates the types for the package. The [`plugin`](./plugin.mdx) uses this CLI command internally.
