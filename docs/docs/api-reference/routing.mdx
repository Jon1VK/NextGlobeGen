...
In rendering contexts the primary way to access the current locale is with [`useLocale`](./hooks.mdx#uselocale) hook, but the locale is also passed as a prop to routing components.
