...
const config: Config = {/* config options here */};
...
const withNextGlobeGen = createNextGlobeGenPlugin({
  configPath: "./configs/next-globe-gen.ts",
});
...
A locale that will be used as a default locale. Should be one of the provided [`locales`](#locales).
...
Be careful when enabling this option. Keys that are used dynamically (e.g., `t(variable)`) cannot be statically extracted and will be pruned. Use [`whitelistedKeys`](#whitelistedkeys) to protect such keys.
