...
```tsx
createNextGlobeGenPlugin(options?: {
  configPath?: string;
  format?: "esm" | "cjs";
});
```
...
| `format`     | `"esm" \| "cjs"` | -        | Format used to compile user files  | `"esm"`            |

The package uses ESM by default. Set `format: "cjs"` when your configuration or imported files require CommonJS.
