...
This function can be used outside of components where the current `locale` cannot be inferred through the rendering context, for example in [`generateMetadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata) or Server Actions.
