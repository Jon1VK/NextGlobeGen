...
async function extractKeys(config: Config) {
  try {
    ...
    const result = await syncMessages(extractedKeys, config);
    const endTime = process.hrtime(startTime);
    const timeDiffInMs = (endTime[0] * 1000 + endTime[1] / 1000000).toFixed(2);
    const msgParts = [
      ...(result.added > 0 ? [`added ${result.added} keys`] : []),
      ...(result.removed > 0 ? [`removed ${result.removed} keys`] : []),
    ];
    const msg =
      msgParts.length > 0
        ? ` - Synced messages: ${msgParts.join(", ")} in ${timeDiffInMs}ms`
        : ` - Message keys are up to date (${timeDiffInMs}ms)`;
    console.info(`\x1b[32mNextGlobeGen\x1b[37m${msg}`);
  } catch (error: unknown) {
    if (error instanceof Error) console.error(error.message);
  }
}

const debouncedExtractKeys = debounce(extractKeys, 250);
